...
UTexture2D * UUINavPCComponent::GetKeyIcon(const FKey Key) const
{
	...
	if (Key.IsGamepadKey())
	{
		...
		{
			KeyIcon = Cast<FInputIconMapping>(GamepadKeyIconData->GetRowMap()[Key.GetFName()]);
		}
	}
	else
	{
		...
		{
			KeyIcon = Cast<FInputIconMapping>(KeyboardMouseKeyIconData->GetRowMap()[Key.GetFName()]);
		}
	}
...
FText UUINavPCComponent::GetKeyText(const FKey Key) const
{
	...
	if (Key.IsGamepadKey())
	{
		...
		{
			Keyname = Cast<FInputNameMapping>(GamepadKeyNameData->GetRowMap()[Key.GetFName()]);
		}
	}
	else
	{
		...
		{
			Keyname = Cast<FInputNameMapping>(KeyboardMouseKeyNameData->GetRowMap()[Key.GetFName()]);
		}
	}
